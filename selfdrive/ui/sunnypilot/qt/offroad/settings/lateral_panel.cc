...
LateralPanel::LateralPanel(SettingsWindowSP *parent) : QFrame(parent) {
  ...
  // Add toggle for speed color change
  speedColorToggle = new ParamControl(
    "SpeedColorChange",
    tr("Enable Speed Color Change"),
    tr("Toggle to enable or disable speed color change based on brake or gas pressed."),
    "../assets/offroad/icon_toggle.png"
  );
  list->addItem(speedColorToggle);
